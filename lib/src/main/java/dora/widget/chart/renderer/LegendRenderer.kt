...
class LegendRenderer(animator: ChartAnimator) : Renderer(animator) {

    val labelPaint = TextPaint(Paint.ANTI_ALIAS_FLAG)
    val iconPaint = Paint(Paint.ANTI_ALIAS_FLAG)
    val textBounds = Rect()

    fun drawLegend(legend: Legend, canvas: Canvas) {
        ...
        for (entry in legend.entries) {
            ...
            val itemHeight = if (legend.type == Legend.LegendType.SQUARE)
                textHeight.coerceAtLeast(legend.iconSize)
            else if (legend.type == Legend.LegendType.CIRCLE) textHeight
